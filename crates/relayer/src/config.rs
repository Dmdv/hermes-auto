...
/// Attempt to load and parse the TOML config file as a `T`.
pub fn load_toml<T>(path: impl AsRef<Path>) -> Result<T, Error>
    where
        T: serde::de::DeserializeOwned
{
    let config_toml = fs::read_to_string(&path).map_err(Error::io)?;

    let config = toml::from_str::<T>(&config_toml[..]).map_err(Error::decode)?;
...
pub fn load_json<T>(path: impl AsRef<Path>) -> Result<T, Error>
    where
        T: serde::de::DeserializeOwned
{
    info!("Trying to load JSON in load_json:{:?}", path.as_ref());

    let config_json = fs::read_to_string(&path).map_err(Error::io)?;

    let config = serde_json::from_str::<T>(config_json.as_str()).map_err(Error::json_decode)?;
...
pub fn store_toml<T>(config: &T, path: impl AsRef<Path>) -> Result<(), Error>
    where
        T: serde::ser::Serialize,
{
    ...
    store_writer_toml(config, &mut file)
}

/// Serialize the given `Config` as TOML to the given config file.
pub fn store_json<T>(config: &T, path: impl AsRef<Path>) -> Result<(), Error>
    where
        T: serde::ser::Serialize,
{
    let mut file = if path.as_ref().exists() {
    ...
    let content = serde_json::to_string_pretty(config).map_err(Error::json_decode)?;
...
fn store_writer_toml<T>(config: &T, mut writer: impl Write) -> Result<(), Error>
    where
        T: serde::ser::Serialize,
{
