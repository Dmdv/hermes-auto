...
/// Spawns a chain runtime from the chain configuration.
/// Returns the corresponding handle if successful.
pub fn spawn_runtime<Handle: ChainHandle>(
    chain_config: &ChainConfig,
    rt: Arc<TokioRuntime>,
) -> Result<Handle, SpawnError> {

    let handle = match chain_config.r#type {
        ChainType::CosmosSdk => ChainRuntime::<CosmosSdkChain>::spawn::<Handle>(chain_config.clone(), rt),
    }.map_err(SpawnError::relayer)?;
