...
define_error! {
    RegistryError {
        ...
        JsonLoadError
            [ TraceError <serde_json::Error> ]
            |_| { format_args!("Error when parsing JSON") },
        ...
        UnhealthyEndpoints
            { endpoints: Vec<String>, retries: u8, chain: String }
            |e| {
                let endpoints = e.endpoints
                    .iter()
                    .join(", ");

                format!("Error finding a healthy endpoint for `{}` after {} retries. Endpoints: {endpoints}", e.chain, e.retries)
            },
        ...
        GrpcTimeOutError
            { url: String }
            [ TraceError<Elapsed> ]
            |e| { format_args!("Unable to connect to GRPC: {}", e.url) },
        ...
        NoChainFound
            { chain_name: String }
            |e| { format_args!("No chain found in the list of chains: {}", e.chain_name) },
        ...
        InvalidChannelConfig
            |_| { "Invalid chain name" },

        RegistryConfigReadError
            { reason: String }
            |e| { format_args!("Error reading channel config file: {}", e.reason) },

        NoChannelsFound
            |_| { "Not found any channel" },
    }
}
