...
/// Models for serializing and deserializing IBC path JSON data found in the `_IBC/`
/// directory of the registry repository
...
impl ChainIdMap {
    pub fn inverse(&self) -> HashMap<String, String> {
        ...
        for (k, v) in self.content.clone().into_iter() {
            inverse.insert(v, k);
        }
        inverse
    }
...
pub struct ChannelPathInfo {
    #[serde(default)]
    pub channels: Vec<IBCPathInfo>,
    #[serde(default)]
    pub chain: ChainShortInfo,
}

#[derive(Default, Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
#[serde(default)]
pub struct ChainShortInfo {
    #[serde(default)]
...
#[derive(Default, Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
#[serde(default)]
pub struct IBCPathInfo {
    #[serde(default)]
    pub chain: ChainShortInfo,
    #[serde(default)]
    pub source: ChannelConfig,
    #[serde(default)]
    pub target: ChannelConfig,
}
