...
/// Obtain a home dir.
///
/// Panics if the home dir was not set.
pub fn app_home() -> PathBuf {
    match APPLICATION.config_path() {
        Some(path) => {
            info!("Getting HOME from config path: {:?}", path);
            path.parent().unwrap().to_path_buf().canonicalize().unwrap()
        },
        None => {
            info!("Getting HOME from config path: {:?}", DEFAULT_CONFIG_PATH);
            PathBuf::from(DEFAULT_CONFIG_PATH).parent().unwrap().to_path_buf()
        },
    }
}
...
impl Application for CliApp {
    ...
    fn after_config(&mut self, config: Self::Cfg) -> Result<(), FrameworkError> {
        ...
        info!("running Hermes v{}", clap::crate_version!());
