...
///
/// Returns the corresponding handle if successful.
pub fn spawn_runtime(config: &ChainConfig, rt: Option<Arc<TokioRuntime>>) -> Result<impl ChainHandle, Error> {
    spawn_runtime_generic::<BaseChainHandle>(config, rt)
}
...
///
/// Returns the corresponding handle if successful.
pub fn spawn_runtime_generic<Handle: ChainHandle>(
    config: &ChainConfig,
    rt: Option<Arc<TokioRuntime>>,
) -> Result<Handle, Error> {
    if let Some(rt) = rt {
        spawn::spawn_runtime(config, rt).map_err(Error::spawn)
    } else {
        let rt = Arc::new(TokioRuntime::new().unwrap());
        spawn::spawn_runtime(config, rt).map_err(Error::spawn)
    }
