use crate::chain_registry_ex::get_configs;
...
pub struct AutoCmd {
    ...
    #[clap(
        long = "only",
        required = false,
        value_name = "from chain",
        help_heading = "REQUIRED",
        help = "Name of the chain from which the chain configs will be generated. The chain must be in the chain registry."
    )]
...
impl Runnable for AutoCmd {
    fn run(&self) {
        ...
        let from_source_chain = self.only_from.clone();
        ...
        info!("Using HOME directory from config file: {:?}", home_dir);

        match runtime.block_on(get_configs(&sorted_names, from_source_chain, commit)) {
            ...
            Err(e) => {
                match &self.only_from {
                    Some(chain) => {
                        error!("Failed to fetch chain configs for source chain `{}` in AutoCmd", chain);
                        error!("{}", e.to_string());
                    }
                    None => {
                        error!("Failed to fetch chain configs for one or every chains in AutoCmd");
                        error!("{}", e.to_string());
                    }
                }
                Output::error("HINT: Try to find healthy endpoint for the specified chain or update it's configuration in github ").exit();
            }
        };
    }
}
